...
const LISTEN_PORT = Number(process.env.PORT || 15722);
...
const SAVE_LAST_SANITIZED_REQUEST = process.env.SAVE_LAST_REQUEST === "1";
const SAVE_LAST_RESPONSE = process.env.SAVE_LAST_RESPONSE === "1";
const LAST_SANITIZED_REQUEST_PATH = path.join(
  __dirname,
  "cc-tencent-last-sanitized-request.json",
);
const LAST_UPSTREAM_RESPONSE_PATH = path.join(
  __dirname,
  "cc-tencent-last-upstream-response.sse",
);
const LAST_NORMALIZED_RESPONSE_PATH = path.join(
  __dirname,
  "cc-tencent-last-normalized-response.sse",
);
...
function writeLastSanitizedRequest(body) {
  if (!SAVE_LAST_SANITIZED_REQUEST) {
    return;
  }

  try {
    fs.writeFileSync(LAST_SANITIZED_REQUEST_PATH, body);
  } catch (error) {
    console.error(`write_last_sanitized_request_error=${error.message}`);
  }
}
...
function writeDebugFile(filePath, content) {
  ...
  try {
    ...
    console.error(`write_debug_file_error=${error.message}`);
  }
}
...
function serializeCollapsedSseChunk(template, choices) {
  const output = {
    ...createSseTemplate(template),
    choices,
  };
  output.usage = output.usage ?? null;
...
function collapseOpenAiSseToolCallStream(streamText) {
  ...
  if (contentChoices.length > 0) {
    output += serializeCollapsedSseChunk(template, contentChoices);
  }
  if (toolChoices.length > 0) {
    output += serializeCollapsedSseChunk(template, toolChoices);
  }
...
function pipeNormalizedOpenAiSse(upstreamRes, res) {
  if (process.env.COLLAPSE_TENCENT_STREAM !== "0") {
    ...
    upstreamRes.on("end", () => {
      ...
      writeDebugFile(LAST_UPSTREAM_RESPONSE_PATH, raw);
      writeDebugFile(LAST_NORMALIZED_RESPONSE_PATH, normalized);
      res.end(normalized);
    });
    return;
  }
...
async function handleRequest(req, res) {
  ...
  if (
    ...
    try {
      ...
      writeLastSanitizedRequest(body);
