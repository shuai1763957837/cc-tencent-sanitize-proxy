...
// 把上游 SSE 流折叠成单个非流式 OpenAI chat.completion JSON 响应体。
// 用于「客户端发非流式、proxy 强制转流式发上游」的场景：腾讯
// /v2/chat/completions 不支持非流式请求（返回 400 code:11101），proxy 注入
// stream:true 转发，再把上游 SSE 折叠回非流式 JSON，对客户端保持透明。
// 复用本地的 collapseOpenAiSseStream，继承其 [Tool use interrupted] 与
// 缓存统计的全部精修，避免重复实现。
function collapseOpenAiSseToNonStreamJson(streamText) {
  const sse = collapseOpenAiSseStream(streamText);
  const template = (() => {
    ...
    for (const choice of parsed.choices || []) {
      ...
      if (Array.isArray(delta.tool_calls)) {
        for (const tc of delta.tool_calls) {
          const idx =
            Number.isInteger(tc.index) || typeof tc.index === "number"
              ? tc.index
              : toolCalls.size;
          const acc = toolCalls.get(idx) || { index: idx, function: {} };
          if (typeof tc.id === "string" && acc.id === undefined) acc.id = tc.id;
          if (typeof tc.type === "string" && acc.type === undefined) {
            acc.type = tc.type;
          }
  ...
  if (toolCalls.size > 0) {
    message.tool_calls = [...toolCalls.values()]
      ...
      .map(({ index, ...rest }) => rest);
    if (finishReason === null) choice.finish_reason = "tool_calls";
  }
  if (functionCall) {
    message.function_call = functionCall;
    if (finishReason === null) choice.finish_reason = "function_call";
  }

  const result = createSseTemplate(template);
  result.object = "chat.completion";
  result.choices = [choice];
  result.usage = normalizeUsage(usage) ?? null;
  return JSON.stringify(result);
}

function pipeCollapsedToNonStream(upstreamRes, res) {
  const chunks = [];
  upstreamRes.on("data", (chunk) => chunks.push(chunk));
  upstreamRes.on("end", () => {
    const raw = Buffer.concat(chunks).toString("utf8");
    const json = collapseOpenAiSseToNonStreamJson(raw);
    writeDebugFile("RESPONSE (normalized)", json);
    if (!res.headersSent) {
      res.writeHead(200, {
...
function forwardRequest(req, res, upstreamUrl, body, logInfo) {
  ...
  if (logInfo.normalizeResponse || logInfo.forcedStream) {
  ...
  const upstreamReq = client.request(
    ...
    (upstreamRes) => {
      ...
      if (logInfo.forcedStream && isSse) {
        // 客户端发非流式、proxy 强制转流式：折叠 SSE 回非流式 JSON。
        // 不预写上游响应头，交由 pipeCollapsedToNonStream 写 application/json 头，
        // 避免头(text/event-stream)与体(JSON)不一致。
        ...
        if (logInfo.normalizeResponse && isSse) {
...
async function handleRequest(req, res) {
  ...
  if (
    ...
    // 非流式请求强制转流式：腾讯 /v2/chat/completions 不支持非流式请求
    // （返回 400 code:11101），注入 stream:true 转发，再折叠回非流式响应。
    try {
      const parsed = JSON.parse(body.toString("utf8"));
  ...
  forwardRequest(req, res, upstreamUrl, body, {
    ...
    bytesDelta,
    rules,
    // 被强制改流式的请求：上游响应原样折叠为非流式 JSON，不做 SSE 归一化
    normalizeResponse: target && isCcswitchPath && !forcedStream,
    forcedStream,
  });
}
